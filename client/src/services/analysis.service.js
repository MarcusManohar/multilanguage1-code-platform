...
/**
 * Analyze student code with AI for actionable feedback
 * @param {Object} params
 * @param {string} params.language
 * @param {string} params.code
 * @returns {Promise<Object>}
...
export async function analyzeCodeWithAI({ language, code }) {
  const url = `${API_BASE_URL}/api/analysis/run-ai`;
  ...
  if (!response.ok) {
    const errorMessage =
      data?.error?.message || data?.message || `AI analysis request failed (${response.status})`;
    const error = new Error(errorMessage);
    error.status = response.status;
    error.data = data;
    throw error;
